...
namespace AIT.TFS.SyncService.Model.TestReport
{
    ...
    /// </summary>
    public class TestSpecificationReportByQueryModel : ExtBaseModel
    {
        ...
        private readonly ITfsTestAdapter _testAdapter;
        private IWord2007TestReportAdapter _importTestReport;
        ...
        public bool CanImport
        {
            get
            {
                return FoundWorkItems != null
                    ...
                    && !IsImporting
                    && !IsFinding
                    && !DocumentModel.OperationInProgress
        ...
        public bool CanFindWorkItems
        {
            get
            {
                if (IsImporting || IsFinding || DocumentModel.OperationInProgress)
                {
                    return false;
                }
                ...
                switch (ImportOption)
                {
                    ...
                    case QueryImportOption.TitleContains:
                        if (!string.IsNullOrEmpty(ImportTitleContains))
                        {
                            return true;
                        }
                        break;
        ...
        private QueryDefinition FindQueryInHierarchy(IEnumerable<QueryItem> queryFolder, string queryPath)
        {
            ...
            foreach (var item in queryFolder)
            {
                ...
                if (item is QueryFolder && FindQueryInHierarchy(item as QueryFolder, queryPath)!=null)
                {
                   return FindQueryInHierarchy(item as QueryFolder, queryPath);
                }
            }
            return null;
        }
        ...
        /// <summary>
        /// Gets all work items that are part of the test case category as test case details.
        /// </summary>
        /// <param name="workItems">The work items.</param>
        /// <param name="expandSharedSteps">Flag to identify if shared steps shall be expanded within test cases.</param>
        /// <returns>A dictionary that maps the IDs of the (test case) work items to their corresponding test case detail object.</returns>
        public Dictionary<int, ITfsTestCaseDetail> GetTestCasesFromWorkItems(IEnumerable<SynchronizedWorkItemViewModel> workItems, bool expandSharedSteps)
        {
            var testCases = new Dictionary<int, ITfsTestCaseDetail>();
            var categories = TfsService.CategoryCollection;
            if (categories == null)
            {
                return testCases;
            }

            var testCaseCategory = categories.FirstOrDefault(c => c.ReferenceName == "Microsoft.TestCaseCategory");
            if (testCaseCategory == null)
            {
                return testCases;
            }

            var testCaseWorkItemTypes = testCaseCategory.WorkItemTypes.Select(wt => wt.Name);
            foreach (var testCaseWorkItem in workItems.Where(w => testCaseWorkItemTypes.Contains(w.TfsItem.WorkItemType)))
            {
                var testCase = _testAdapter.TestManagement.TestCases.Find(testCaseWorkItem.Id);
                if (testCase != null)
                {
                    testCases[testCaseWorkItem.Id] = new TfsTestCaseDetail(new TfsTestCase(testCase), expandSharedSteps);
                }
            }
        ...
        /// <summary>
        /// Gets all work items that are part of the shared steps category.
        /// </summary>
        /// <param name="workItems">The work items.</param>
        /// <returns>A dictionary that maps the IDs of the (shared steps) work items to their corresponding detail object.</returns>
        public Dictionary<int, ITfsSharedStepDetail> GetSharedStepsFromWorkItems(IEnumerable<SynchronizedWorkItemViewModel> workItems)
        {
            var sharedSteps = new Dictionary<int, ITfsSharedStepDetail>();
            var categories = TfsService.CategoryCollection;
            if (categories == null)
            {
                return sharedSteps;
            ...
            var sharedStepCategory = categories.FirstOrDefault(c => c.ReferenceName == "Microsoft.SharedStepCategory");
            if (sharedStepCategory == null)
            {
                return sharedSteps;
            }

            var sharedStepsWorkItemTypes = sharedStepCategory.WorkItemTypes.Select(wt => wt.Name);
            foreach (var sharedStepsWorkItem in workItems.Where(w => sharedStepsWorkItemTypes.Contains(w.TfsItem.WorkItemType)))
            {
                var sharedStepWorkItem = _testAdapter.TestManagement.SharedSteps.Find(sharedStepsWorkItem.Id);
                if (sharedStepWorkItem != null)
                {
                    sharedSteps[sharedStepsWorkItem.Id] = new TfsSharedStepDetail(new TfsSharedStep(sharedStepWorkItem));
                }
            }
